    ...
    module Commands
      module Feature
        ...
        def feature_close(name)
          print "Preparing to close a feature for app "
          print @app.teal
          print " named "
          puts name.teal
          puts
          ...
          if status.include? "Changes not staged for commit:"
            raise ExecutionError.new "Your current branch has unstaged changes. Please "\
              "commit or stash them before closing the feature."
          end

          branches = `git branch -a`
          unless branches.include? ("feature/#{name}\n")
            raise ExecutionError.new "No feature named '#{name}' could be found "\
          ...
          exec "git branch -d feature/#{name}"
